...
(function(global) {
  ...
  CompoundObserver.prototype = createObject({
    ...
    check_: function(changeRecords, skipChanges) {
      ...
      for (var i = 0; i < this.observed_.length; i += 2) {
        ...
        // THIS IS A HACK! Dirty checking workspaces massively degraded performance, so we don't.
        if (this.value_ != undefined){
          if (this.value_[0] instanceof Array){
            if (this.value_[0].length > 0){
              if (this.value_[0][0].id == "sprintplanning2" || this.value_[0][0].id == "taskboard"){
                return false;
              }
            }
          }
  ...
  function ObserverTransform(observable, getValueFn, setValueFn,
    ...
    // at the moment because of a bug in its dependency tracking.
